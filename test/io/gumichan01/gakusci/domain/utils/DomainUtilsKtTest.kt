...
internal class DomainUtilsKtTest {
    ...
    fun `Take elements from list`(): Unit {
        val response = SearchResponse(1, 0, listOf(SimpleResultEntry("", "")))
        assertThat(response.take(1).entries).contains(SimpleResultEntry("", ""))
    }

    @Test
    fun `Retrieve sublist of list - normal case`(): Unit {
        val response = SearchResponse(1, 0, listOf(SimpleResultEntry("", "")))
        assertThat(response.slice(IntRange(0, 0)).entries).contains(SimpleResultEntry("", ""))
    }

    @Test
    fun `Retrieve sublist bigger than the list - return the list `() {
        val response = SearchResponse(2, 0, listOf(SimpleResultEntry("a", "b"), SimpleResultEntry("", "")))
    ...
    fun `Retrieve sublist at the end of the list with non-zero start - return the sublist `() {
        val response = SearchResponse(2, 0, listOf(SimpleResultEntry("a", "b"), SimpleResultEntry("", "")))
        assertThat(response.slice(IntRange(1, 4)).entries).isEqualTo(listOf(SimpleResultEntry("", "")))
    }
}