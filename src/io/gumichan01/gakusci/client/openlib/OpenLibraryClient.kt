...
class OpenLibraryClient : IClient<OpenLibraryResponse> {
    ...
    override suspend fun retrieveResults(queryParam: QueryParam): OpenLibraryResponse? {
        val url = openLibUrl.format(queryParam.query)
        return retrieveData(url)
    }

    private suspend fun retrieveData(url: String): OpenLibraryResponse? {
        return try {
    ...
    private fun String.fromJson(): OpenLibraryResponse = jacksonObjectMapper().readValue(this)
}
